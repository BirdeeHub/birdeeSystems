...
// rec {
  ...
  birdIsAttrs =
    v: isAttrs v.value && !isDerivation v.value && !isFunctor v.value && !hasOutPath v.value;
  ...
  recAttrsToList =
    ...
    flip pipe [
      ...
      (foldl' (
        a: v: if birdIsAttrs v.value then a ++ (recAttrsToList v.path v.value) else a ++ [ v ]
      ) [ ])
    ];
  ...
  eachSystem =
    ...
    foldl' op { } (
      ...
      ++ (
        if
          builtins ? currentSystem && !elem builtins.currentSystem systems
