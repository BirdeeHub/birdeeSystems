{
  ...
  flake.wrappers.zsh = { pkgs, wlib, lib, config, ... }@top: {
    ...
    config.zshAliases = let
      ...
      nh = wlib.evalPackage [
        ...
        ({ config, wlib, ... }: {
          ...
          config.flake = top.config.flake-path;
    ...
    in {
      ${if config.flake-path != null && config.output-name != null then "rebuild-system" else null} = lib.getExe nh;
      ${if config.flake-path != null && config.output-name != null then "rebuild-home" else null} = lib.getExe (nh.wrap { eval-type = "home"; });
      ...
      yeet = "rm -rf";
      ccd = ''cd "$(${inputs.self.wrappers.xplr.wrap { inherit pkgs; }}/bin/xplr --print-pwd-as-result)"'';
