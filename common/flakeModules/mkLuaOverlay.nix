...
let
  ...
  in {
    ...
    ${if prev.lib.isFunction vimPlugins || builtins.isList vimPlugins then "vimPlugins" else null} = prev.vimPlugins // (
      if builtins.isList vimPlugins then
        (prev.lib.composeManyExtensions (prev.lib.flatten vimPlugins)) final prev
      else
