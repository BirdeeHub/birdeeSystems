    ...
    }:
    {
      ...
      config.prefixVar = [
        [
          "LUA_PATH"
          ":"
          "${config.luaEnv.pkgs.luaLib.genLuaPathAbsStr config.luaEnv}"
        ]
        [
          "LUA_CPATH"
          ":"
          "${config.luaEnv.pkgs.luaLib.genLuaCPathAbsStr config.luaEnv}"
        ]
        [
          "LUA_PATH"
          ":"
          "${placeholder "out"}/cfg/?.lua;${placeholder "out"}/cfg/?/init.lua"
        ]
        [
          "LUA_CPATH"
          ":"
          "${placeholder "out"}/cfg/?.so"
        ]
      ];
    };
}
