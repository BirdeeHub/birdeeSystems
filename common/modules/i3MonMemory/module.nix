...
let
  module =
      ...
      mkUserXrandrScript =
        ...
        (
          ...
          pkgs.writeShellScript "${scriptName}" (
            ...
            ''
            + (
              if cfg.monitorScriptDir != null && builtins.pathExists "${cfg.monitorScriptDir}/${scriptName}" then
                builtins.readFile "${cfg.monitorScriptDir}/${scriptName}"
              else
                builtins.readFile ./defaults/${scriptName}
            )
          )
        );
    ...
    in
    {
      homeManager = {
        _file = ./module.nix;
        options.${moduleNamespace}.i3MonMemory = {
          ...
          monitorScriptDir = lib.mkOption {
            ...
            description =
              (lib.literalExpression ''
                ...
                i.e.
                ''${config.monitorScriptDir} ━┳━━ XmonBoot.sh
                        ┣━━ Xprimary.sh
                        ┗━━ Xothers.sh
        ...
        config = lib.mkIf config.${moduleNamespace}.i3MonMemory.enable {
...
in
{
  flake.modules.nixos.i3MonMemory = module;
  flake.modules.homeManager.i3MonMemory = module;
}
