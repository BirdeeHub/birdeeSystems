{ config, pkgs, lib, wlib, ... }: {
  ...
  config.flags."--config" = "${placeholder "out"}/${config.binName}-rc.lua";
  config.package = lib.mkDefault pkgs.i3status;
  config.drv.passAsFile = [ "i3barcfg" ];
  config.drv.buildPhase = ''
    runHook preBuild
    { [ -e "$i3barcfgPath" ] && cat "$i3barcfgPath" || echo "$i3barcfg"; } > ${lib.escapeShellArg "${placeholder "out"}/${config.binName}-rc.lua"}
    runHook postBuild
  '';
  config.drv.i3barcfg = ''
    order += "load"
