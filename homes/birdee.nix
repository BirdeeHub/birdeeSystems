...
in {
  ...
  home.shellAliases = {
    ...
    yoloAI = ''git add . && git commit -m "$(ollama run llama3.1 generate a silly commit message. reply with ONLY the commit message and nothing else. Do not wrap the entire reply in any sort of surrounding characters such as ` " () [] or '"'"' (using surrounding characters internally is fine)')" -m '(auto-msg llama3.1)' -m "$(git status)" && git push'';
